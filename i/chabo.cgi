...
# ==================== 管理者が設定する項目です（必須） ====================
$SCRIPT     = 'http://kanetanet.sakura.ne.jp/sachigiku.com/i/chabo.cgi';  # chabo.cgi自身を呼び出すURL
$BOARD_NAME = '噂佐菊都風あたまとり掲示板';          # メッセージボード名称(PC用)
$BOARD_NAM2 = '噂佐菊都風あたまとり掲示板';         # メッセージボード略称(携帯用 全角８文字以内推奨)

$HOMEPAGE   = 'http://blog.sachigiku.com/';       # ホームページのアドレス
$HOMEPAG2   = 'http://blog.sachigiku.com/';         # ホームページのアドレス(iモード用)
#$HOMEPAG3   = '';        # ホームページのアドレス(EZweb用)
#$HOMEPAG4   = '';         # ホームページのアドレス(J-SkyWeb用)
#$HOMEPAG5   = '';         # ホームページのアドレス(-H"/PメールDX用)
#$HOMEPAG6   = '';        # ホームページのアドレス(ドットi用)		(Ver3.20)
...
$CSS_URL   = 'http://kanetanet.sakura.ne.jp/sachigiku.com/i/chabo.css';    # CSSファイルのURL					(Ver3.20)


# ======= ChaBo Call 関係の初期設定です（必ず、正しく設定して下さい） ======
$CHABOCALL = 1;                                 # ChaBo Call機能(0:使わない 1:使う)
$SENDMAIL  = '/usr/sbin/sendmail';               # サーバ内のsendmailのあるパス
$ML_TO     = 'kaneta.kohei@gmail.com';        # ChaBo Call受信メールアドレス
$ML_FROM   = 'kaneta.kohei@gmail.com';             # サーバからの送信メールアドレス
...
$ACT_LINK  = 0;					# クリッカブルメール機能     (0:使わない 1:使う)
$ACT_URL   = 0;					# 簡易クリッカブルＵＲＬ機能 (0:使わない 1:使う)	(Ver3.20)
$ACT_INITIAL = 1;				# あたまとり機能 (0:使わない 1:使う)	(Ver3.20)
$ACT_WORD = 1;					# キーワード機能 (0:使わない 1:使う)	(Ver3.20)
$KEYWORD = '「じょろうぐも」';
$KEYWORD2 = '「いちのたにふたばぐんき」';
$KEYWORD3 = '「きさらず」';
$CUT_TEL   = 1;					# 電話番号カット機能        （0:使わない 1:使う)	(Ver3.20)
$CUT_URL   = 1;					# URLカット機能              (0:使わない 1:使う)	(Ver3.20)
$MAX_MESS  = 100;				# 保存するメッセージ最大数
$MESS_HOME = 'さちぎくブログに戻る';		# ホームページへ戻る旨のリンク表示(PC)
...
# ---通常端末
	...
	# ---組み込みファイルがあれば、表示
		if( open(INC,"<$INCF") ) {
			...
			$mesbuff .= '「あたまとり」をしながらキーワードを当てましょう。<BR>';
			$mesbuff .= '例えば「かぶき」の次は「○○○か」です。<BR>';
			$mesbuff .= '<a href=http://blog.sachigiku.com/2014/07/blog-post_91.html>●●はじめての方はこちらをご覧ください。●●</a>'."\n";
		}
...
<ins class=\"adsbygoogle\"
     ...
     data-ad-client=\"ca-pub-2199441690547589\"
     data-ad-slot=\"9590395096\"></ins>
<script>
...
# ---H"/PDX端末
	...
	# ---メニューページ出力
		if($page == 0) {
			...
			if($HOMEPAG5 ne '') { $mesbuff .= "?$MESS_HOM2\n"; }
		...
		if(!$mode) {
			foreach(@buffer) {
				...
				if($ACT_WORD ) { $message = &active_word($message); }
                                if($ACT_WORD ) { $message = &active_word2($message); }
                                if($ACT_WORD ) { $message = &active_word3($message); }
...
#						$mesbuff .= "/$kosuu";
				...
				if($page-$view > 1) { $mesbuff .= "?次↑\n"; }

				if($page < $kosuu)       { $mesbuff .= "?最初\n"; }
				if($page+$view < $kosuu) { $mesbuff .= "?前↓\n"; }
...
# ---Undoボタン表示（通常端末）
	if(!$mode) {
		...
		# ---ホームページへ戻るリンク表示
		if($HOMEPAGE ne '') {
			$mesbuff .= "<A HREF=\"$HOMEPAGE\">$MESS_HOME</A><BR><BR><!-- Chabo336x280 -->
<ins class=\"adsbygoogle\"
     style=\"display:inline-block;width:336px;height:280px\"
     data-ad-client=\"ca-pub-2199441690547589\"
     data-ad-slot=\"1508725096\"></ins>
<script>
(adsbygoogle = window.adsbygoogle || []).push({});
</script><!-- Chabo336x280 -->
<ins class=\"adsbygoogle\"
...
# ---ページを閉じる
	...
	elsif($mode >= 0 ) { $mesbuff .= "<script>
  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
  })(window,document,'script','//www.google-analytics.com/analytics.js','ga');

  ga('create', 'UA-53196197-1', 'auto');
  ga('send', 'pageview');

</script></BODY></HTML>"; }	# その他(-H"/PDX以外)
...
sub active_word2 {
	local($toencode) = @_[0];	
	$toencode =~ s/($KEYWORD2)/$1<font color="#b22222"> ← 【中当たり】  <\/font>/g;
	return $toencode;
}

sub active_word3 {
	local($toencode) = @_[0];	
	$toencode =~ s/($KEYWORD3)/$1<font color="#b22222"> ← 【小当たり】  <\/font>/g;
	return $toencode;
}
